...
| 2 | **hang/ho/mok 분리** | 위 동일 파일 | `get_article_detail(mst, jo)` | content에 ①②③ 있는 조문은 반드시 `hang[]` 배열 분리. ho도 `ho[]` 배열로 분리. **"다음 각 목" 포함 ho/hang에는 반드시 `mok[]` 배열 존재** |
...
2.5. **[HARD] mok 검증**: "다음 각 목" 포함 ho/hang 스캔 → `get_article_detail`로 목 원본 조회 → mok[] 배열 추가
...
### 2. hang/ho/mok 분리 완전성 — 20점
- content에 ①②③ 등 원문자가 포함된 조문은 반드시 `hang[]` 배열로 분리되어야 함
- hang.number는 숫자 문자열(`"1"`, `"2"`)이어야 함 (원문자 `"①"` 금지)
- **[HARD] "다음 각 목" 불변조건**: ho 또는 hang의 content에 "다음 각 목"이 포함되면 반드시 `mok[]` 배열이 존재해야 함
- mok 형식: `{ "number": "가", "content": "..." }` (한글 가나다 순서)
- 미분리 조문이 있으면 조문번호 목록과 함께 경고
- **검증 스크립트**: 수집 완료 후 전체 JSON에서 `"다음 각 목"` 패턴 스캔 → mok 누락 시 FAIL
